...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        self.gender.grid(row = currentrow+7, column = 0,columnspan=2,sticky=W)
        ...
        self.submit_button.grid(row = currentrow+8, column = 1,columnspan = 2, sticky = W) 
        self.submit_button.configure(font=f)
        self.lock = Label(self, text = 'Lock')
        self.lock.grid(row=currentrow+8,column=3,sticky=W)
        self.char_info = []
        self.char_labels = []
        self.char_info_lock = []
        currentrow = 12
        for i,k in enumerate(info_labels):
            ...
            self.char_info[i].grid(row = currentrow+i, column = 1,columnspan=2, sticky = E) 
            self.char_labels.append(Label(self, text = k+': '))
            self.char_labels[i].grid(row = currentrow+i, column = 0,sticky = E)
            var = IntVar()      
            self.char_info_lock.append(var)
            l = Checkbutton(self,variable=self.char_info_lock[i],onvalue=1,offvalue=0)
            l.grid(row = currentrow+i, column = 3,sticky = W)
        self.stats = []
        self.stat_labels = []
        currentrow = 17
        for i,k in enumerate(stat_names):
            self.stat_labels.append(Label(self, text = k+': '))
            self.stat_labels[i].grid(row = currentrow+i, column = 0, sticky = E) 
            self.stats.append(Entry(self))
            self.stats[i].grid(row = currentrow+i, column = 1,columnspan=2, sticky = E) 
    ...
    def getnames(self):
       ...
       # else: 
        ...
        for n in self.resultant_names:
            ...
            print_info.append("%i"%(self.player_age))         
            height_ft = np.floor(self.player_height) 
            print_info.append("%i"%(height_ft)+'ft '+"%i"%((self.player_height-height_ft)*12)+'in')
            print_info.append("%i"%(self.player_weight)+'lbs')
    ...
    def getinfo(self):
        ...
        if self.player_gender == 'Male':
            ...
            scale = 0.95
        self.player_height = random.gauss(np.mean(self.race_height[self.race_var.get()]),float(np.diff(self.race_height[self.race_var.get()]))/4)*scale
        self.player_weight = random.gauss(np.mean(self.race_weight[self.race_var.get()])+(self.player_height-np.mean(self.race_height[self.race_var.get()]))*30,np.diff(self.race_weight[self.race_var.get()])/4)*scale
